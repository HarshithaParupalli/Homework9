...
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    generate_qr_code(
        request.url, qr_code_full_path, FILL_COLOR, BACK_COLOR, request.size
    )
...
async def list_qr_codes_endpoint(token: str = Depends(oauth2_scheme)):
    ...
    responses = [
        QRCodeResponse(
            ...
            qr_code_url=decode_filename_to_url(
                qr_file[:-4]
            ),  # Decode the filename to the original URL
...
@router.delete(
    "/qr-codes/{qr_fileame}", status_code=status.HTTP_200_OK, tags=["QR Codes"]
)
async def delete_qr_code_endpoint(
    qr_filename: str, token: str = Depends(oauth2_scheme)
):
...
async def generate_qr_code_endpoint(
    request: QRCodeRequest, token: str = Depends(oauth2_scheme)
):
    """
