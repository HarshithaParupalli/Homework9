...
class QRCodeRequest(BaseModel):
    ...
    fill_color: str = Field(default="black", description="QR code color.")
    back_color: str = Field(
        default="white", description="Background color of the QR code."
    )
    size: conint(ge=1, le=40) = Field(default=10, description="Size of the QR code.")
...
class TokenData(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"username": "user@example.com"}}
